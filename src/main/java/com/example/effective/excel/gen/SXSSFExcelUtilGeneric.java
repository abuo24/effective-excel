...
/**
 * Generic Excel Util class to write data to excel with classes that extends {@link AbstractExcelEntity}
 * <p>
 * Author: Orifjon Yunusjonov
 * since: 05/10/2024
 */
@Slf4j
public class SXSSFExcelUtilGeneric {
    ...
    protected <T extends AbstractExcelEntity> void writeToExcel(HttpServletResponse response, List<? extends AbstractExcelEntity> result, Sheet sheet, Integer fromHeader, ExcelBody<? extends AbstractExcelEntity> excelBody) throws IOException {
        int chunkSize = ChunkSizeEnum.getChunkSizeForValue(result.size());
        ProcessDataTaskGeneric<T> task = new ProcessDataTaskGeneric(result, sheet, fromHeader, excelBody, chunkSize);
...
//        }
    ...
    /**
     * Optional method to index result list
     * If you use <b>asynchronous read you must call it<b/>
     */
    protected <T extends AbstractExcelEntity> void putIndex(List<T> result) {
        for (int i = 0; i < result.size(); i++) {
            result.get(i).setExcelIndex(i);
        }
    }
